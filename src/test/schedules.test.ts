...
// ----------------------------------------------------------------
describe('Scheduler getNextRunTime', () => {
  let getNextRunTime: typeof import('../server/scheduler').getNextRunTime;

  beforeEach(async () => {
    vi.resetModules();
    // Mock schedule-store and report-runner to avoid DB deps
    ...
    vi.doMock('../server/report-runner', () => ({
      executeReport: async () => 'mock-id',
    }));
    const mod = await import('../server/scheduler');
    getNextRunTime = mod.getNextRunTime;
  });

  it('returns a valid ISO date for valid cron', () => {
  ...
  it('returns null for invalid cron', () => {
    const result = getNextRunTime('invalid cron');
  ...
  it('handles every-minute cron', () => {
    ...
    expect(nextTime - Date.now()).toBeLessThan(120_000);
  });
});
